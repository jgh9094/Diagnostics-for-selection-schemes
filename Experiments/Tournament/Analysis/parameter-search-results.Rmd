...
50 replicates are conducted for each tournament size `T` parameter value explored.

```{r tor-libs, message=FALSE}
library(ggplot2)
library(cowplot)
library(dplyr)
...
```{r tor-exp-mvc-per-sli, warning=FALSE}
...
Here we present the results for **best performances** found by each tournament selection size value replicate on the ordered exploitation diagnostic.
...
```{r tor-ord-mvc-per-sli, warning=FALSE}
# 80% and final generation comparison
end = filter(tor_ot_mvc, diagnostic == 'ordered_exploitation' & gen == 50000 & T != 'ran')
end$Generation <- factor(end$gen)

mid = filter(tor_ot_mvc, diagnostic == 'ordered_exploitation' & gen == 40000 & T != 'ran')
mid$Generation <- factor(mid$gen)
...
Here we present the results for **satisfactory trait coverage** and **activation gene coverage** found by each tournament selection size value replicate on the ordered exploitation diagnostic.
...
```{r tor-con-mvc-sat-sli, warning=FALSE}
# 80% and final generation comparison
...
```{r tor-con-mvc-act-sli, warning=FALSE}
# 80% and final generation comparison
end = filter(tor_ot_mvc, diagnostic == 'contradictory_objectives' & gen == 50000)
end$Generation <- factor(end$gen)
...
Here we present the results for **best performances** and **activation gene coverage** found by each tournament selection size value replicate on the multi-path exploration diagnostic.
...
```{r tor-mpe-per-end-wil}
pairwise.wilcox.test(x = performance$pop_fit_max, g = performance$T , p.adjust.method = "bonferroni",
                     paired = FALSE, conf.int = FALSE, alternative = 'l')
```
...
```{r tor-mpe-mvc-per-sli, warning=FALSE}
# 80% and final generation comparison
end = filter(tor_ot_mvc, diagnostic == 'multipath_exploration' & gen == 50000 & T != 'ran')
end$Generation <- factor(end$gen)

mid = filter(tor_ot_mvc, diagnostic == 'multipath_exploration' & gen == 40000 & T != 'ran')
mid$Generation <- factor(mid$gen)
...
```{r tor-mpe-mvc-act-sli, warning=FALSE}
# 80% and final generation comparison
