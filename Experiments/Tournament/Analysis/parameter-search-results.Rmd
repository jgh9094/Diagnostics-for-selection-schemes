...
Here we present the results for the **best performances** and **best gene value** generated by each tournament selection replicate on the multi-valley crossing diagnostic.
Best performance found refers to the largest average trait score found in a given population.
...
```{r tor-mvc-per-ot, warning=FALSE}
lines = filter(tor_ot, diagnostic == 'multivalley_crossing') %>%
  group_by(T, gen) %>%
  dplyr::summarise(
    min = min(pop_fit_max),
    mean = mean(pop_fit_max),
...
```{r tor-mvc-per-bst}
best = filter(tor_best, col == 'pop_fit_max' & diagnostic == 'multivalley_crossing')
plot =  ggplot(best, aes(x = T, y = val / TRAITS, color = T, fill = T, shape = T)) +
          geom_flat_violin(position = position_nudge(x = .2, y = 0), scale = 'width', alpha = 0.2) +
          geom_point(position = position_jitter(width = .1), size = 1.5, alpha = 1.0) +
          geom_boxplot(color = 'black', width = .2, outlier.shape = NA, alpha = 0.0) +
...
```{r tor-mvc-per-bst-sum}
best$T = factor(best$T, levels = c(2,4,8,16,32,64,128,256))
group_by(best, T) %>%
  dplyr::summarise(
    count = n(),
    na_cnt = sum(is.na(val)),
...
```{r tor-mvc-per-end}
end = filter(tor_end, diagnostic == 'multivalley_crossing')
...
```{r tor-mvc-per-end-sum}
end$T = factor(end$T, levels = c(2,4,8,16,32,64,128,256))
group_by(end, T) %>%
  dplyr::summarise(
    count = n(),
...
```{r tor-mvc-gne-ot, warning=FALSE}
lines = filter(tor_ot, diagnostic == 'multivalley_crossing') %>%
  group_by(T, gen) %>%
  dplyr::summarise(
...
```{r tor-mvc-gne-bst}
best = filter(tor_best, col == 'pop_max_gene' & diagnostic == 'multivalley_crossing')
plot =  ggplot(best, aes(x = T, y = val, color = T, fill = T, shape = T)) +
          geom_flat_violin(position = position_nudge(x = .2, y = 0), scale = 'width', alpha = 0.2) +
          geom_point(position = position_jitter(width = .1), size = 1.5, alpha = 1.0) +
          geom_boxplot(color = 'black', width = .2, outlier.shape = NA, alpha = 0.0) +
...
```{r tor-mvc-gne-end-wil}
pairwise.wilcox.test(x = end$pop_max_gene, g = end$T, p.adjust.method = "bonferroni",
                     paired = FALSE, conf.int = FALSE, alternative = 'l')
```