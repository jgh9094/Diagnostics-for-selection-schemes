...
A new set of candidate solutions must also be selected as genetic material, usually from the existing population of candidate solutions, for the following generation.
...
From this point on, this process will be defining a selection scheme.
These descriptions were inspired from [1,2].

The selection scheme that an evolutionary algorithm uses plays an important role in finding an optimal solution.
It selects starting points to move from within the space of all solutions, that translates to some performance from the fitness landscape.
Mutation operators and percentages act as the direction and distance candidate solutions use to traverse the solution space.
...
Looking at the other extreme, traversing from a terrible point in the solution space can lead to candidate solutions being stuck in an unfavorable region.

A selection scheme returns a list of candidate solutions to be used as genetic material for the following generation.
Prior to mutation, each one of these candidate solutions can be thought of as a single starting point in the solution space.
...
Given that the same candidate solution to produce an offspring multiple times, it is also easy to see that it is beneficial to have multiple candidate solutions explore the same region around a good starting point in the solution space.
...
Below we elaborate on the four components: fitness modification, population structure, selection, and external selection.


## Fitness Modification

Before a selection scheme can start selecting candidate solutions for the following generations, each candidate solution must have a single or set of fitness scores.
...
Let $\rho(j,t)$ define a function that takes a candidate solution and testcase as input and returns a fitness performance value, where $\rho : (\mathbb{J},\mathbb{T}) \longrightarrow \mathbb{R}$.
...
For example, the first row represents $j_1$'s performance on all testcases.
...
Let $F$ represent the matrix that has undergone some transformation.
...
Looking at $F$ we can define $f_i = g(x_i)$, where the function $g$ applies some transformation to the raw performance fitness values. 
...
$$f_i = g(x_i) = h_1 \bigg ( \sum_{x \in x_i} h_2(x) \bigg ) $$
...
$$f_i = g(x_i) = \{ h(x_{(i,1)}), h(x_{(i,2)}), ..., h(x_{(i,M)}) \}$$
...
$$F = G(F)$$
  - $G$ represents some matrix transformation on $F$
...
**The key take from this section is that once we have the raw performance fitness values on all testcases from each candidate solution, additional layers of complexity can be applied to the data.**
Once transformations on data are complete, if any are applied, the selection scheme must determine how to use these fitness values.
...
While those that reside within the same cohort all have interactions between each other.

We will describe standard tournament selection in this context. 
Standard tournament selection randomly picks a predetermined number of candidate solutions from the entire population, then selections the top performing solution. 
...
Tournament selection is then used to selection candidate solutions, but only allow tournaments with solutions from the same cohort.
...
Some simple approaches could be always select the best performing candidate solution or randomly select solutions. 
These are nowhere near the optimal methods to selection candidate solutions.
