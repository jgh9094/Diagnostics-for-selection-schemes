...
In this work, we are focusing on the **selection scheme**. 

# Selection Schemes 

Below we describe in detail, with analysis, the different existing selection schemes. 
...
As $\mu$ approaches $\lambda$, we get this algorithm to resemble more of a drift selection scheme that focuses on exploring the fitness landscape.
...
As $t$ approaches 0,  we get this algorithm to resemble more of a drift selection scheme that focuses on exploring the fitness landscape.
...
One major thing worth noting is that when all candidate solutions are at least $\sigma$ apart from one another, this selection scheme reduces down to an algorithm that focuses on how it selects candidate solutions for the following generation (step 4).
...
This selection scheme posses a two features that highlight its novelty: keeping testcase performace seperate and filtering with testcases to find a parent.
It uses all testscases independently, in a random order, to filter down to a parent for the following generation.
Other selection schemes typically aggregate the performance of all the testcases per candidate solution. 
